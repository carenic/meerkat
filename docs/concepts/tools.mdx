...
generation at `inherit`, which the outer factory treats as hidden; they do not
yet expose an image-generation enable switch. `--allow-tool generate_image`
only narrows an already composed surface and cannot enable the category.
