...
owns execution and terminal result, while a terminal evidence projector adds a
typed reference to the item and asks WorkGraph to close it under its existing
completion policy. A scheduled job likewise leaves time with Schedule and
execution with the jobs subsystem.
