...
<Steps>
  ...
  <Step title="Choose feature flags">
    ...
    [dependencies]
    meerkat = { version = "0.8.24", features = ["sqlite-store", "session-store"] }
    ```
...
<Note>
`turn_completed` carries usage for one provider call while `run_completed`
carries the session-cumulative total, and their `input_tokens` fields use
different denominators. Read
