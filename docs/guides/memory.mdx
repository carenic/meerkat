...
|---|---|---|---|
| `auto_compact_threshold` | `u64` | `100_000` | Compaction triggers when `last_input_tokens >= threshold` OR `estimated_history_tokens >= threshold` OR the measured whole-request token budget crosses the threshold. |
...
<Steps>
  ...
  <Step title="Threshold evaluation">
    ...
    - `estimated_history_tokens >= auto_compact_threshold` (JSON bytes of all messages / 4), OR
    - the measured request budget for this boundary crosses the threshold on its
      ...
      active model declares a context window.
  </Step>
</Steps>
