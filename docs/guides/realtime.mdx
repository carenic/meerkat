...
async with MeerkatClient() as client:
    ...
    # 2. Open a live channel.
    channel = await client.live_open(session.id)
    # channel.transport_bootstrap contains the WebSocket URL
    # channel.capabilities describes supported input/output modalities

    # 3. Send audio input and commit at turn boundaries.
    await client.live_send_input_audio(session.id, audio_data)
    await client.live_commit_input(session.id)

    # 4. Close the channel when done.
    await client.live_close(session.id)
