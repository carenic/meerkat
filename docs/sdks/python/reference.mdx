...
One `Usage` type serves two different accounts. On the `TurnCompleted` event it
is per-call usage: `input_tokens` is the raw provider counter (for Anthropic,
