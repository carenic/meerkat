...
<ParamField body="handling_mode" type="String">
  Required for `kind="peer_message"` and `kind="peer_request"`. Use `"queue"` for ordinary delivery or `"steer"` for immediate steer processing on runtime-backed sessions.
</ParamField>

<Note>
Use `peer_message` for normal agent collaboration. Use `peer_request` only when you want an explicit structured ask and later `peer_response` correlated by `in_reply_to`.
</Note>
