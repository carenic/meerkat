...
- activation: `preload_skills`, `skill_refs`, or an explicitly policy-enabled
  `load_skill` tool
...
- **CLI preload convenience**: `rkat run --skill <skill-name>` materializes a
  builtin-source key from a bare slug. It does not currently address a local
  filesystem source discovered from a path.
...
These five tools are default-disabled in `SkillToolSet`. A custom
...
Collection mode groups by `source_uuid` and currently instructs the agent to
...
The public prompt and wire contracts never encode identity as a slash-delimited
string that another component must parse.
