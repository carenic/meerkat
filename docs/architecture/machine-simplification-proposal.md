...
| Behavior-bearing runtime-control state should stay outside MeerkatMachine | rejected | `current_run_id` is now absorbed back into the checked-in Meerkat machine, the stale handwritten control booleans are gone, and both the live recycle path and the dead handwritten recover workflow have been removed from the helper. The remaining work is the duplicated run-return bookkeeping plus the compatibility-facing `Recovering`/`Resume` surface, not keeping live shell state below the two-machine model. |
