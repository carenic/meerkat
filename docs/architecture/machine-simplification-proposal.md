...
| Helper-owned admission wake/process signaling should stay below the checked-in machine | rejected / landed | Removed admission-side `WakeRuntime` / `InterruptYielding` / `RequestImmediateProcessing` ownership from `RuntimeIngressAuthority`. The runtime now computes the immediate post-admission control signal directly from the resolved policy before executing an explicit ingress transition, so the helper no longer decides the first control consequence of a newly accepted input. All three Meerkat parity audits stayed green. |
