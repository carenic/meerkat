...
If the built-in summarization request itself is rejected as over-context or
too large, Meerkat uses a deterministic mechanical handoff summary so the
session can shed old rows and continue. Ordinary LLM errors still emit
`compaction_failed` and preserve the original transcript. A configured curator
never falls back to the mechanical or LLM path.
