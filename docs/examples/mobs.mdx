...
```bash
rkat mob spawn-helper release-triage "Join as lead-1 and summarize the release status." --profile lead --agent-identity lead-1 --result-label release-summary --max-text-bytes 4096
