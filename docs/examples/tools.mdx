...
Built-in tool categories: builtins (`enable_builtins`, including task tools plus utility helpers like `datetime`, `apply_patch`, and `view_image`), shell (`enable_shell`), semantic memory (`enable_memory`), and mob orchestration (`enable_mob`).
