...
| `session/runtime_state` | Runtime | Read runtime-backed ingress state |
...
| `session/accept_input` | Runtime | Admit external work directly into runtime ingress |
| `session/retire_runtime` | Runtime | Retire runtime-backed ingress |
| `session/reset_runtime` | Runtime | Reset runtime-backed ingress |
| `session/input_state` | Runtime | Read runtime input lifecycle state |
...
<Note>
`session/runtime_state`, `session/accept_input`, `session/retire_runtime`, `session/reset_runtime`, `session/input_state`, `session/inputs` are advertised only when the server is running with the runtime-backed ingress/control-plane surface enabled.
</Note>
...
```json Response
{
  ...
  "result": {
    ...
    "methods": [
      ...
      "session/runtime_state", "session/realtime_attachment_status", "session/realtime_attachment_statuses", "session/accept_input", "session/retire_runtime", "session/reset_runtime",
