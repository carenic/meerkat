...
| `auth/login/start` | Auth | Begin interactive OAuth (follow-up commit) |
| `auth/login/complete` | Auth | Finish interactive OAuth (follow-up commit) |
| `auth/login/device_start` | Auth | Begin device-code OAuth (follow-up commit) |
| `auth/status/get` | Auth | Get auth status for a profile |
| `auth/logout` | Auth | Revoke + remove persisted credentials (CLI-driven for now) |
