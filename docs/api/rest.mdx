...
<Steps>
  <Step title="Start the server">
    ...
    ```bash Live channel proxy
    rkat-rpc --realm team-alpha --tcp 127.0.0.1:9001 --live-ws 127.0.0.1:9002
    rkat-rest --realm team-alpha --live-rpc-tcp 127.0.0.1:9001
    ```
    </CodeGroup>
  </Step>
...
| `POST` | `/sessions/{id}/live/open` | Open a live channel on a session |
| `POST` | `/sessions/{id}/live/close` | Close a live channel |
| `GET` | `/sessions/{id}/live/status` | Fetch live channel status |
...
| `POST` | `/live/open` | Open a live channel (proxied to configured `rkat-rpc` live host) |
| `POST` | `/live/status` | Read live channel status |
| `POST` | `/live/close` | Close a live channel |
...
Live channels provide low-latency audio and text streaming. Create a session
on a realtime-capable model (e.g. `gpt-realtime-2`), then open a live channel:

```bash
curl -X POST http://127.0.0.1:8080/sessions \
  -H 'content-type: application/json' \
  -d '{"prompt":"Ready for live interaction.","model":"gpt-realtime-2","provider":"openai"}'
```

Then open the channel:

```bash
curl -X POST http://127.0.0.1:8080/sessions/{id}/live/open \
  -H 'content-type: application/json' \
  -d '{}'
```

See the [Live Channels guide](/guides/realtime) for the full flow.

### POST /live/open

Open a live channel, proxied to the configured `rkat-rpc --tcp ... --live-ws ...` host.

```bash
curl -X POST http://127.0.0.1:8080/live/open \
  -H 'content-type: application/json' \
  -d '{"session_id":"01936f8a-7b2c-7000-8000-000000000001"}'
```

`/live/status` and `/live/close` use `{"session_id":"..."}`.
If `--live-rpc-tcp` is not configured on `rkat-rest`, these
