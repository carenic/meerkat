...
mob_SubmitWorkRunning(arg_agent_runtime_id, arg_fence_token, arg_work_id, arg_origin) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes \cup { [route |-> "work_request_reaches_meerkat", source_machine |-> "mob", effect |-> "RequestRuntimeIngress", target_machine |-> "meerkat", target_input |-> "Ingest", payload |-> [origin |-> packet.payload.origin, runtime_id |-> packet.payload.agent_runtime_id, work_id |-> packet.payload.work_id], actor |-> "meerkat_kernel", effect_id |-> (model_step_count + 1), source_transition |-> "SubmitWorkRunning"] }
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "RequestRuntimeIngress", payload |-> [agent_runtime_id |-> packet.payload.agent_runtime_id, fence_token |-> packet.payload.fence_token, origin |-> packet.payload.origin, work_id |-> packet.payload.work_id], effect_id |-> (model_step_count + 1), source_transition |-> "SubmitWorkRunning"] }
...
CoreNext ==
    ...
    \/ \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_work_id \in WorkIdValues : \E arg_origin \in StringValues : mob_SubmitWorkRunning(arg_agent_runtime_id, arg_fence_token, arg_work_id, arg_origin)
