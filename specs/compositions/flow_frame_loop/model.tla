...
flow_frame__AnyDepCompleted(node_id) == (\E dep_id \in SeqElements((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed"))

flow_frame__AllDepsCompleted(node_id) == (\A dep_id \in SeqElements((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed"))

flow_frame__NodeAdmissionEligible(node_id) == (IF (Len((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) = 0) THEN TRUE ELSE (IF ((IF node_id \in DOMAIN flow_frame_node_dependency_modes THEN flow_frame_node_dependency_modes[node_id] ELSE "None") = "All") THEN (\A dep_id \in SeqElements((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) : (((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Canceled"))) ELSE ((\E dep_id \in SeqElements((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed")) \/ (\A dep_id \in SeqElements((IF node_id \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[node_id] ELSE <<>>)) : (((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Canceled"))))))
...
flow_frame_AdmitNextReadyNode_Skip ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ flow_frame_phase = "Running"
       /\ (Len(flow_frame_ready_queue) > 0)
       /\ ((((IF Head(flow_frame_ready_queue) \in DOMAIN flow_frame_node_dependency_modes THEN flow_frame_node_dependency_modes[Head(flow_frame_ready_queue)] ELSE "None") = "All") /\ (\E dep_id \in SeqElements((IF Head(flow_frame_ready_queue) \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[Head(flow_frame_ready_queue)] ELSE <<>>)) : (((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Canceled")))) \/ ((IF Head(flow_frame_ready_queue) \in DOMAIN flow_frame_node_branches THEN flow_frame_node_branches[Head(flow_frame_ready_queue)] ELSE None) \in flow_frame_branch_winners))
       /\ flow_frame_phase' = "Running"
       /\ flow_frame_last_admitted_node' = Head(flow_frame_ready_queue)
...
flow_frame_AdmitNextReadyNode_Fail ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ flow_frame_phase = "Running"
       /\ (Len(flow_frame_ready_queue) > 0)
       /\ (((IF Head(flow_frame_ready_queue) \in DOMAIN flow_frame_node_dependency_modes THEN flow_frame_node_dependency_modes[Head(flow_frame_ready_queue)] ELSE "None") = "Any") /\ (\A dep_id \in SeqElements((IF Head(flow_frame_ready_queue) \in DOMAIN flow_frame_node_dependencies THEN flow_frame_node_dependencies[Head(flow_frame_ready_queue)] ELSE <<>>)) : (((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN flow_frame_node_status THEN flow_frame_node_status[dep_id] ELSE "None") = "Canceled"))) /\ ~(flow_frame__AnyDepCompleted(Head(flow_frame_ready_queue))))
       /\ flow_frame_phase' = "Running"
       /\ flow_frame_last_admitted_node' = Head(flow_frame_ready_queue)
...
CoreNext ==
    ...
    \/ \E arg_step_id \in StepIdValues : \E arg_target_id \in MeerkatIdValues : \E arg_retry_key \in StringValues : flow_run_RecordTargetFailure(arg_step_id, arg_target_id, arg_retry_key)
