...
29. **The capability-driven realtime-attachment plane is gone.** `session/realtime_attachment_status` (and its proposed plural sibling), `mob/member_status.realtime_attachment_status`, `realtime/open_info` / `realtime/status` / `realtime/capabilities`, `RealtimeAttachmentStatus`, and `MeerkatMachine::reconfigure_live_topology` were deleted in the live-adapter MVP. Live channels are now caller-initiated through `live/*` (`live/open`, `live/status`, `live/refresh`, `live/send_input`, `live/commit_input`, `live/interrupt`, `live/truncate`, `live/close`); `ModelCapabilities.realtime` only gates whether a model can back a live channel. Stragglers referencing the old surfaces in older docs/audit notes do not constitute a contract.
