...
pub fn render_machine_kernel_module(schema: &MachineSchema) -> String {
    ...
    match module_name.as_str() {
        "meerkat" => return render_meerkat_modeled_module(),
        "mob" => return render_canonical_stub_modeled_module(schema),
        "schedule_lifecycle" => return render_canonical_stub_modeled_module(schema),
        "occurrence_lifecycle" => return render_canonical_stub_modeled_module(schema),
        "auth" => return render_canonical_stub_modeled_module(schema),
        _ => {}
    }
